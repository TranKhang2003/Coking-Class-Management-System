...
-- Roles Table
	CREATE TABLE Users (
		...
		Email NVARCHAR(255) unique-- Mật khẩu người dùng
	);
...
-- Chèn vai trò vào bảng Roles (nếu chưa có)
...
-- Lấy ID của người dùng 'admin'
DECLARE @UserId INT;
SELECT @UserId = id FROM Users WHERE HoTen = N'admin';

-- Lấy ID của vai trò 'admin'
DECLARE @RoleId INT;
SELECT @RoleId = id FROM Roles WHERE RoleName = N'admin';
...
AS
BEGIN
    INSERT INTO HocVien (HocVien.user_id, HoTen, GioiTinh, NgaySinh, NgayDangKy, GhiChu, Sdt)
    VALUES (@user_id, @HoTen, @GioiTinh, @NgaySinh,@NgayDangKy, @GhiChu, @Sdt);
END;
go
...
BEGIN
    ...
    SELECT @Email = Email, @HoTen = HoTen, @MatKhau = MatKhau FROM inserted;
    ...
    -- Chèn dữ liệu nếu tất cả điều kiện đều hợp lệ
    INSERT INTO Users (HoTen, MatKhau, Email)
    SELECT HoTen, MatKhau, Email FROM inserted;
END;

----------------------------------------------------------------
----------------------------------------------------------------
--Bổ Sung PROCEDURE
--tìm kiếm kể cả từ không có dấu 
create proc P_ViewMonAnByConvert (@str nvarchar(255))
as
begin 
select * from MonAn where dbo.fn_ConvertToUnsign1(MonAn.TenMon) like N'%' + dbo.fn_ConvertToUnsign1(@str) + '%' ;
end;
